...
//! invokes the dispatcher from Rust — no per-message `Promise` or op re-entry,
//! mirroring Node's `uv_async` "wake, drain, emit" model. Dispatch still runs
//! an explicit microtask checkpoint between messages to preserve the observable
//! task boundary that the old async receive loop provided.
//!
